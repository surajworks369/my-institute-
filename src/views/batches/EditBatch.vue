<template>
  <div class="batch-form-page fade-in">
    ...
    <div v-if="!batchFound" class="not-found card">Batch not found.</div>

    <div v-else class="batch-form-layout">
      <div class="form-card card">
        <form class="batch-form" @submit.prevent="submit">
          <div class="form-grid">
            ...
            <div class="form-group">
              ...
              <select v-model="form.assistantTrainer" class="form-control">
                ...
                <option v-for="trainer in assistantTrainerOptions" :key="trainer" :value="trainer">
                  {{ trainer }}
                </option>
              </select>
            </div>
          ...
          <div class="form-actions">
            <router-link to="/batches" class="app-btn app-btn-secondary">Cancel</router-link>
            <button type="submit" class="app-btn app-btn-primary">Update Batch</button>
          </div>
        </form>
      </div>
      ...
      <div class="side-panel">
        ...
        <div class="tips-card card">
          ...
          <ul class="tips-list">
            <li>Keep course mapping correct for cross-module consistency.</li>
            <li>Enrolled count cannot be greater than capacity.</li>
            <li>Batch updates affect related academic workflows.</li>
          </ul>
        </div>
      </div>
    </div>
...
const assistantTrainerOptions = [...trainerOptions]
...
onMounted(() => {
  ...
  const batch = batchStore.getById(id)

  if (!batch) {
    batchFound.value = false
    return
  }

  form.name = batch.name
  form.code = batch.code
  form.courseId = batch.courseId
  form.startDate = batch.startDate
  form.endDate = batch.endDate
  form.timing = batch.timing
  form.days = [...batch.days]
  form.trainer = batch.trainer
  form.assistantTrainer = batch.assistantTrainer ?? ''
  form.capacity = batch.capacity
  form.enrolled = batch.enrolled
  form.classroom = batch.classroom ?? ''
...
const safeDaysText = computed(() => {
  return form.days.length ? form.days.join(', ') : '—'
})
