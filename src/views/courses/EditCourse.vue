<template>
  <div class="course-form-page fade-in">
    ...
    <div v-if="!courseFound" class="not-found card">Course not found.</div>

    <div v-else class="form-card card">
      <form class="course-form" @submit.prevent="handleUpdate">
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <input :value="currentEnrolled" type="number" class="form-control" disabled />
          </div>
        ...
        <div class="form-actions">
          <router-link to="/courses" class="app-btn app-btn-secondary">Cancel</router-link>
          <button type="submit" class="app-btn app-btn-primary">Update Course</button>
        </div>
      </form>
    </div>
  </div>
...
const { getCourseById, updateCourse, init } = courseStore

const id = Number(route.params.id)
const courseFound = ref(true)
...
onMounted(() => {
  ...
  form.batch = course.batch
  form.trainer = course.trainer
  form.assistantTrainer = course.assistantTrainer
  currentEnrolled.value = course.enrolledStudents
})
