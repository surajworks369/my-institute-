    ...
    <div class="form-layout">
        ...
        <form class="exam-form" @submit.prevent="handleSubmit">
          <div class="form-grid">
            <div class="form-group">
              ...
              <input v-model.trim="form.name" class="form-control" placeholder="Enter exam name" />
              <small v-if="errors.name" class="error-text">{{ errors.name }}</small>
            </div>

            <div class="form-group">
              <label>Exam Code</label>
              <input v-model.trim="form.code" class="form-control" placeholder="Enter exam code" />
          ...
          <div class="form-actions">
            ...
            <button type="submit" class="app-btn app-btn-primary">Save Exam</button>
          </div>
        </form>
      </div>
      ...
      <div class="preview-stack">
        <div class="preview-card card">
          ...
          <div class="preview-box">
            ...
            <div class="preview-grid">
              ...
              <div class="preview-item">
                <label>Marks</label>
                <p>{{ form.passingMarks }}/{{ form.totalMarks }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="preview-card card">
          <h3>Guidelines</h3>
          <ul class="tips-list">
            <li>Select course first, then only linked batch.</li>
            <li>Passing marks cannot be greater than total marks.</li>
            <li>Time range should be valid for proper scheduling.</li>
          </ul>
        </div>
      </div>
    </div>
...
const form = reactive<ExamCreateInput>({
  name: '',
  code: '',
  courseId: 0,
  batchId: 0,
...
watch(
  () => form.courseId,
  () => {
    const batch = batchStore.getById(form.batchId)
    if (!batch || batch.courseId !== form.courseId) {
      form.batchId = 0
    }
  },
)
...
const selectedCourseName = computed(
  () => courseStore.getCourseById(form.courseId)?.name ?? 'Select Course',
)
const selectedBatchName = computed(() => batchStore.getById(form.batchId)?.name ?? 'Select Batch')
...
function validate() {
  ...
  if (!form.code.trim()) errors.code = 'Exam code is required'
  ...
  if (form.courseId && form.batchId && !examStore.validateExamLink(form.courseId, form.batchId)) {
...
function resetForm() {
  ...
  form.examDate = todayYMD()
  form.startTime = '10:00'
  form.endTime = '12:00'
  form.totalMarks = 100
  form.passingMarks = 35
  form.instructions = ''
  clearErrors()
}
...
function handleSubmit() {
  ...
  const created = examStore.createExam({
    ...form,
    name: form.name.trim(),
  ...
  if (!created) {
    errors.batchId = 'Unable to create exam. Please verify course and batch mapping.'
    return
  }
